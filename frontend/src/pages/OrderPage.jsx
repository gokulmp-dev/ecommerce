...
const OrderPage = () => {
  ...
  const handleReturn = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage(`${returnType === 'return' ? 'Return' : 'Replacement'} request submitted!`)
  ...
  const handleReview = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/orders/${id}/review`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user.token}`
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {message && (
        <div className="bg-green-100 text-green-700 px-4 py-3 rounded-xl mb-4 text-center font-semibold">
          {message}
        </div>
      )}
      {error && (
        <div className="bg-red-100 text-red-600 px-4 py-3 rounded-xl mb-4 text-center">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <div className="flex justify-between font-bold text-lg">
            <span>Total:</span>
            <span className="text-yellow-500">${order.totalPrice.toFixed(2)}</span>
          </div>
        </div>
      </div>
        ...
        {showReturnForm && (
          <div className="mt-4 border rounded-xl p-4 bg-gray-50">
            ...
            <textarea
              ...
              className="border px-4 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-400 mb-3"
              rows={3}
              value={returnReason}
        ...
        {showReviewForm && (
          <div className="mt-4 border rounded-xl p-4 bg-gray-50">
            ...
            <textarea
              ...
              className="border px-4 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-yellow-400 mb-3"
              rows={3}
              value={comment}
      ...
      <button
        onClick={() => navigate('/')}
        className="mt-6 w-full bg-gray-900 text-white py-3 rounded-lg hover:bg-yellow-500 transition font-semibold"
      >
        Continue Shopping
      </button>
    </div>
